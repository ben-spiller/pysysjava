	...
	def parse(self):
		...
		The testsuite dictionary contains keys:
			...
			* ``stdout: str`` (optional) - Any text written to stdout by all testcases in the suite 
			  (stripped of leading/trailing whitespace) for dialects such as Ant which don't provide this per-testcase.
		...
		if len(self.results)==0:
			assert self.suite.get('skipped') >= 1, 'Test suite contains no results, but skipped=0: %s'%self.path
			# Create a fake result so that it shows up. NB: JUnit 5 always generates a result so this is just for Ant
			self.results.append({
