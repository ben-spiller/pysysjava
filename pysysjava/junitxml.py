	...
	def parse(self):
		...
		if len(self.results)==0:
			if 'uniqueId' in self.suite:
				...
				# a way to detect that the suite is from the JUnit5 launcher rather than an actual test class
				log.debug('Ignoring suite "%s" with no results generated by the JUnit5 launcher')
			else:
